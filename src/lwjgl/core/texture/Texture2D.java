...
import lwjgl.core.ContextValues;
...
public class Texture2D extends GLObject {
	...
	public void setLOD(float min, float max) {
		bind();
	...
	public void setData(int w, int h, int lod, int texformat, ImageFormat format, ImageDataType type, ByteBuffer data) {
		if (w < 0 || h < 0) {
			Logging.glError("Cannot set data of Texture2D [" + name + "] with dimensions (" + w + "," + h
					+ "). Dimensions must be non-negative.", this);
			return;
		}
		int max = ContextValues.intConst(GL11.GL_MAX_TEXTURE_SIZE);
		if (w > max || h > max) {
			Logging.glError("Cannot set data of Texture2D [" + name + "] with dimensions (" + w + "," + h
					+ "). Device only supports textures up to (" + max + "," + max + ").", this);
			return;
		}
		GL11.glTexImage2D(target.value, lod, texformat, w, h, 0, format.value, type.value, data);
	}
	
	public void setData(BufferedImage src, int lod, int texformat) {
		setData(src.getWidth(), src.getHeight(), lod, texformat, ImageFormat.RGBA, ImageDataType.UBYTE,
	...
	public String[] status() {
		...
		status.add(Logging.logText("Texture2D:", name, 0));
		...
		status.add(Logging.logText(String.format("%-16s:\t%.3f - %.3f", "LOD Range", lodmin, lodmax), 1));
